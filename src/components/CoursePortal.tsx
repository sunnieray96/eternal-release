...
export default async function CoursePortal() {
  ...
  return (
    <section id="courses" className="py-48 md:py-80 bg-white relative overflow-hidden text-left">
        ...
        <div className="grid md:grid-cols-3 gap-20 md:gap-32">
          {courses.length === 0 ? (
            <>
              ...
              <Pathway title="The Whole System Flow" level="Advanced" desc="Advanced neuroplasticity flow to build new neural pathways for resilience." img="https://customer-assets.emergentagent.com/wingman/29fab97e-3d97-4ee3-a7c2-df0f13706430/attachments/d88bb604e1054353ae9dcd0ccc20d03b_pexels-darina-belonogova-9167188.jpg" />
            </>
          ) : (
            courses.map((c: any) => (
               <Pathway key={c._id} title={c.title} level={c.level} desc={c.description} img={c.mainImage} />
            ))
          )}
        </div>
      </div>
   ...
   return (
      <div className="space-y-16 group">
         ...
         <div className="text-left">
            ...
            <h3 className="font-serif text-4xl md:text-5xl italic text-black mb-10 leading-none">{title}</h3>
            <p className="text-lg font-light text-black/60 leading-relaxed text-balance">{desc}</p>
            <span className="inline-block mt-16 text-[10px] font-bold uppercase tracking-[0.6em] border-b border-black/10 pb-1">Coming Soon</span>
         </div>
      </div>
   )
}
