...
export default function ImmersiveSanctuary() {
  ...
  const storyOpacity = useTransform(scrollYProgress, [0.25, 0.3, 0.45, 0.5], [0, 1, 1, 0]);
  const storyScale = useTransform(scrollYProgress, [0.25, 0.3, 0.5], [0.95, 1, 1.05]);

  const scienceOpacity = useTransform(scrollYProgress, [0.5, 0.55, 0.7, 0.75], [0, 1, 1, 0]);
  const scienceScale = useTransform(scrollYProgress, [0.5, 0.55, 0.75], [0.95, 1, 1.05]);

  const pathwaysOpacity = useTransform(scrollYProgress, [0.75, 0.8, 1], [0, 1, 1]);
  const pathwaysScale = useTransform(scrollYProgress, [0.75, 0.8, 1], [0.95, 1, 1]);
  ...
  return (
    <div ref={containerRef} className="relative h-[800vh] bg-bg">
      <motion.div style={{ backgroundColor: bgColor }} className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
              ...
              {SCENES.map((s) => (
                 <div key={s.id} className="flex items-center gap-10 group">
                    ...
                    <div className="text-left">
                       ...
                       <p className={`font-serif text-lg italic transition-all duration-500 mt-2 ${activeScene === s.id ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}>{s.title}</p>
                    </div>
                 </div>
              ))}
           </div>
        ...
        <motion.div style={{ opacity: heroOpacity, scale: heroScale }} className="absolute inset-0 flex items-center justify-center p-8 z-10">
           <div className="max-w-7xl mx-auto text-center">
              ...
              <h1 className="font-serif text-6xl md:text-[11rem] font-light leading-[0.8] tracking-tighter mb-20 text-ink">Your body knows<br /><span className="italic font-extralight text-moss opacity-40">the way home</span></h1>
           ...
           <div className="absolute inset-0 -z-10 opacity-[0.08]"><img src="https://customer-assets.emergentagent.com/wingman/29fab97e-3d97-4ee3-a7c2-df0f13706430/attachments/dbcab09ef71446b09f928c042620df92_pexels-angela-roma-7479889.jpg" className="w-full h-full object-cover" alt="Hero" /></div>
        </motion.div>

        <motion.div style={{ opacity: storyOpacity, scale: storyScale }} className="absolute inset-0 flex items-center justify-center p-8 md:pl-96 md:pr-32 z-20">
        ...
        <motion.div style={{ opacity: scienceOpacity, scale: scienceScale }} className="absolute inset-0 flex items-center justify-center p-8 md:pl-96 md:pr-32 z-30">
           <div className="flex flex-col md:flex-row gap-24 items-center max-w-7xl">
              <div className="flex-1 space-y-16 text-left">
                 ...
                 <p className="text-2xl md:text-3xl font-light text-black/60 leading-relaxed max-w-lg">Science confirms that trauma isn&apos;t just a thought. It is a physical bracing in your connective tissue. We use somatic movement to signal safety to your Vagus nerve.</p>
              </div>
              <div className="flex-1 w-full hidden md:block">
                 <div className="aspect-[3/4] rounded-[120px] overflow-hidden border border-sand/20 relative"><img src="https://customer-assets.emergentagent.com/wingman/29fab97e-3d97-4ee3-a7c2-df0f13706430/attachments/cf32f8b8961541939f851db417ec776a_pexels-daria-liudnaya-8187528.jpg" className="w-full h-full object-cover opacity-60 grayscale" alt="Anatomy" /></div>
              </div>
           </div>
        </motion.div>

        <motion.div style={{ opacity: pathwaysOpacity, scale: pathwaysScale }} className="absolute inset-0 flex items-center justify-center p-8 md:pl-96 md:pr-32 z-40">
           <div className="max-w-7xl w-full text-left">
              <div className="mb-32 text-left">
                 <p className="text-[10px] uppercase tracking-[0.4em] font-bold text-black opacity-30 mb-8">Teachings</p>
                 <h2 className="font-serif text-6xl md:text-[11rem] font-light leading-none tracking-tighter text-black">Pathways</h2>
              </div>
              <div className="grid md:grid-cols-3 gap-12 md:gap-32">
...
function PathwayCard({ title, level, desc, img }: any) {
   return (
      <div className="space-y-12 opacity-60 hover:opacity-100 transition-all duration-700">
         <div className="aspect-[4/5] overflow-hidden rounded-[80px] border border-sand/20"><img src={img} className="w-full h-full object-cover grayscale" alt={title} /></div>
         <div>
            <p className="text-[9px] uppercase tracking-[0.4em] font-bold text-clay mb-6 font-sans">{level}</p>
            <h3 className="font-serif text-4xl italic text-black mb-8 leading-tight">{title}</h3>
            <p className="text-lg font-light text-black/60 leading-relaxed font-sans">{desc}</p>
         </div>
      </div>
   )
}
